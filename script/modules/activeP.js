...
function activeP(event) {
    ...
    values.activePBlock.style.border = "white solid 2px"
    document.querySelector(".open-note_screen").addEventListener("mousemove", moveP)
    values.activePBlock.addEventListener("mouseup", unactiveP)
    values.mouseXY.X = event.clientX
    values.mouseXY.Y = event.clientY
    console.log(values.activePBlock, values.mouseXY.X, values.mouseXY.Y)
}

export default activeP