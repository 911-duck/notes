...
function moveP(event) {
    let x = values.mouseXY.X - event.clientX
    let y = values.mouseXY.Y - event.clientY
    openRightBoardEditor(values.activePBlock)
    values.activePBlock.style.top = values.activePBlock.offsetTop - y + "px"
    values.activePBlock.style.left = values.activePBlock.offsetLeft - x + "px"
    values.mouseXY.X = event.clientX
    values.mouseXY.Y = event.clientY
}

export default moveP